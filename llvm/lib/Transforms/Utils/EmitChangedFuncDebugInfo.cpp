...
static bool getArgs(unsigned NumArgs, BasicBlock &FirstBB, DIBuilder &DIB,
...
begin:
  for (Instruction &I : FirstBB) {
    for (const DbgRecord &DR : I.getDbgRecordRange()) {
      ...
      if (dyn_cast<AllocaInst>(MDNValue)) {
        /* AllocaInst, struct/union type in debuginfo, and ptr to struct/union in
         * actual code.
         */
          ...
          if (!Ty->isIntegerTy(DTy->getSizeInBits())) {
            ...
            for (auto Op : Expr->expr_ops()) {
              ...
              for (auto *Element : DTy->getElements()) {
                ...
                if (Elem->getSizeInBits() == BitSize && Elem->getOffsetInBits() == BitOffset) {
                  ParamType = Elem->getBaseType();
                  Found = true;
                  break;
                } else {
                  // Create a new int type. For example, original debuginfo is an array.
                  ParamType = DIB.createBasicType("int" + std::to_string(BitSize),
  ...
  /* The parameter is not handled due to poison value, so just create type */
  ...
  if (Ty->isIntegerTy()) {
    ...
    DILocalVariable *Var = DIB.createParameterVariable(OldSP, F->getArg(Idx)->getName(),
    ...
    DIType *ParamType = DIB.createBasicType("int" + std::to_string(IntBitWidth),
        IntBitWidth, dwarf::DW_ATE_signed);
    ParamType = DIB.createPointerType(ParamType, 64);
    DILocalVariable *Var = DIB.createParameterVariable(OldSP, F->getArg(Idx)->getName(),
                                      Idx + 1, NewFile, OldSP->getLine(),
                                      ParamType);
    TypeList.push_back(ParamType);
...
static bool getTypeArgList(DIBuilder &DIB, DIFile *NewFile, Function *F,
  ...
  if (!getArgs(NumArgs, FirstBB, DIB, NewFile, F, OldSP, TypeList, ArgList))
    return false;
