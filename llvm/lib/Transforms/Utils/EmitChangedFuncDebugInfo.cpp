...
static DIType *getIntTypeFromExpr(DIBuilder &DIB, DIExpression *Expr,
                                  ...
                                  int &CoerceVal, Argument *Arg, unsigned Idx) {
      ...
      if (auto *Elem = dyn_cast<DIDerivedType>(Elems[i])) {
        if (N >= 2 && i < N - 1) {
          if (Elem->getOffsetInBits() <= BitOffset &&
            ...
            CoerceVal = BitOffset >= 0 && BitOffset < PointerBitWidth ? 0 : 1;
...
static DIType *computeParamDIType(DIBuilder &DIB, Type *Ty, DIType *Orig,
                                  ...
                                  bool ByVal, Function *F, int &CoerceVal,
                                  Argument *Arg, unsigned Idx) {
  ...
  return getIntTypeFromExpr(DIB, Expr, Comp, W, PointerBitWidth, F, CoerceVal,
                            Arg, Idx);
}
...
static bool getOneArgDI(unsigned Idx, BasicBlock &Entry, DIBuilder &DIB,
  ...
  int CoerceVal = -1; // 0: suffix __coerce0; 1: suffix __coerce1
  DIType *ParamType =
      computeParamDIType(DIB, ArgTy, DIVar->getType(), PointerBitWidth, DIExpr,
                         !ByValTypeName.empty(), F, CoerceVal, Arg, Idx);
  if (!ParamType)
    return false;
  ...
  std::string VarName;
  if (ArgName.empty()) {
    VarName = DIVar->getName();
    if (CoerceVal == 0 || CoerceVal == 1)
      VarName += "__coerce" + std::to_string(CoerceVal);
...
static void generateDebugInfo(Module &M, Function *F,
  ...
  if (cc != dwarf::DW_CC_nocall) {
    ...
    // If the function is generated with FunctionSpecializer, its signature
    // will be different from the source function. So this special function
    // name is encoded for dwarf func name directly.
    if (F->getName().find(".specialized.")) {
      FuncName = F->getName();
    } else {
...
PreservedAnalyses EmitChangedFuncDebugInfoPass::run(Module &M,
  ...
  for (Function &F : M) {
    ...
    bool FuncHasDot = F.getName().contains('.');
    if (SkipDottedFunc && FuncHasDot)
      continue;

    if (!FuncHasDot && SP->getType()->getCC() != dwarf::DW_CC_nocall)
      continue;
