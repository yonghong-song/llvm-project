...
void DwarfDebug::addChangedSubprograms() {
  ...
  NamedMDNode *NMD = MMI->getModule()->getNamedMetadata("llvm.dbg.sp.extra");
  if (!NMD)
    return;
  for (const DISubprogram *SP : ProcessedSPNodes) {
    for (MDNode *Node : NMD->operands()) {
      DISubprogram *ExtraSP = cast<DISubprogram>(Node);
      if (ExtraSP->getScope() != SP)
        continue;

      DwarfCompileUnit &Cu = getOrCreateDwarfCompileUnit(SP->getUnit());
      DIE *ScopeDIE = DIE::get(DIEValueAllocator, dwarf::DW_TAG_inlined_subroutine);
      ...
      if (ExtraSP->getType()->getCC() == llvm::dwarf::DW_CC_nocall) {
        Cu.addUInt(*ScopeDIE, dwarf::DW_AT_calling_convention, dwarf::DW_FORM_data1,
      ...
      for (unsigned i = 1, N = Args.size(); i < N; ++i) {
        ...
        if (!Ty) {
          ...
          Cu.addType(Arg, Ty);
          if (Ty->isArtificial())
            Cu.addFlag(Arg, dwarf::DW_AT_artificial);
        }
      }
    }
  }
